<div>
    <div class="row mb-3">
        ...
        <div class="col-md-2 col-sm-12 mb-2">
            <div class="d-grid">
                <a href="{{ route('admin.ledger.withdraws.create') }}"
                                ...
                                @foreach ($ledgers as $withdraw)
                                    <tr wire:key="{{ $withdraw->id }}">
                                        ...
                                        <td>
                                            ...
                                            {{ currencyFormat($withdraw->withdraw) }}
                                        </td>
                                        <td>
                                        ...
                                        <td class="text-right">
                                            @if ($showDeleted)
                                                @can('ledger.restore')
                                                    <button wire:click="confirmRestore({{ $withdraw->id }})"
                                                ...
                                                @can('ledger.force.delete')
                                                    <button wire:click="confirmForceDelete({{ $withdraw->id }})"
                                            ...
                                            @else
                                                @can('ledger.read')
                                                    <a href="{{ route('admin.ledger.withdraws.show', $withdraw->id) }}"
                                                        class="btn btn-sm btn-outline-info">
                                                        <i class="ri-eye-line"></i>
                                                    </a>
                                                @endcan
                                                @can('ledger.update')
                                                    <a href="{{ route('admin.ledger.withdraws.edit', $withdraw->id) }}"
