                        ...
                        <table id="data" class="table table-striped table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th style="width: 30%">Payment Detail</th>
                            ...
                            <tbody>
                                @foreach ($paymentMethods as $paymentMethod)
                                    <tr wire:key="{{ $paymentMethod->id }}">
                                        ...
                                        <td>
                                            <input type="checkbox" wire:change="toggleStatus({{ $paymentMethod->id }})"
                                                id="is_active_{{ $paymentMethod->id }}" switch="bool"
                                                {{ $paymentMethod->is_active ? 'checked' : '' }} />
                                            <label for="is_active_{{ $paymentMethod->id }}" data-on-label="Yes"
                                                data-off-label="No"></label>
                                        </td>
                                        <td class="text-right">
                                            @if ($showDeleted)
                                                @can('payment-method.restore')
                                                    <button wire:click="confirmRestore({{ $paymentMethod->id }})"
                                                        class="btn btn-sm btn-outline-info" data-toggle="modal"
                                                        data-target="#deleteModal">
                                                        <i class="ri-arrow-go-back-line"></i>
                                                    </button>
                                                @endcan
                                                @can('payment-method.force.delete')
                                                    <button wire:click="confirmForceDelete({{ $paymentMethod->id }})"
                                                ...
                                                @endcan
                                            @else
                                                @can('payment-method.read')
                                                    <a href="{{ route('admin.payment-methods.show', $paymentMethod->id) }}"
                                                        class="btn btn-sm btn-outline-info">
                                                        <i class="ri-eye-line"></i>
                                                    </a>
                                                @endcan
                                                @can('payment-method.update')
                                                ...
                                                @can('payment-method.delete')
                                                    <button wire:click="confirmDelete({{ $paymentMethod->id }})"
                                                        class="btn btn-sm btn-outline-danger" data-toggle="modal"
                                                        data-target="#deleteModal">
