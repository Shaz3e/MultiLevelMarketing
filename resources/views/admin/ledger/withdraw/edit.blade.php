...
@section('content')
    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <form action="{{ route('admin.ledger.withdraws.update', $withdraw) }}" method="POST" class="needs-validation" novalidate>
                    @csrf
                    @method('put')
                    <div class="card-body">
                        <div class="row mb-3">
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="user_id" id="user_id" class="form-control">
                                        ...
                                        @if ($withdraw->user)
                                            <option value="{{ $withdraw->user_id }}" selected>
                                                {{ $withdraw->user->name }}
                                            </option>
                                        @endif
                                    </select>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <select name="payment_method_id" id="payment_method_id" class="form-control">
                                        <option value="">Select</option>
                                        @if ($withdraw->payment_method_id)
                                            <option value="{{ $withdraw->payment_method_id }}" selected>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" id="withdraw" name="withdraw"
                                        value="{{ old('withdraw', $withdraw->withdraw) }}" required>
                                </div>
                                @error('withdraw')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                                ...
                                <div class="form-group">
                                    ...
                                    <select name="status" id="status_name" class="form-control">
                                        @foreach ($ledgerStatuses as $status)
                                            <option value="{{ $status }}" {{ $status == $withdraw->status ? ' selected' : ''}}>{{ $status }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @error('status')
